...
- **Champ Pays à l'Inscription (détection auto + dropdown) & Répartition Géographique (super-admin) :** Le formulaire d'inscription ([`src/app/(auth)/register/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/(auth)/register/page.tsx)) affiche désormais un menu déroulant Pays (étape 2, alimenté par `countriesData`), pré-rempli automatiquement via géolocalisation IP (`detectCountry()` dans [`src/lib/api.ts`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/lib/api.ts), qui interroge `GET /auth/detect-country/`) mais toujours librement modifiable par l'utilisateur avant soumission. `countriesData.ts` gagne un champ `continent` par pays et un helper `getContinent()`. Côté dashboard super-admin ([`src/app/super-admin/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/super-admin/page.tsx)), nouveau widget **« Répartition géographique »** affichant la répartition des clients par pays et par continent (barres proportionnelles) à travers toutes les instances, avec mise en avant explicite du pays comptant le plus de clients.
- **Suppression des Produits de Démonstration Auto-Générés (`/stock`, `/products`) :** Les pages Stock ([`src/app/(dashboard)/stock/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/(dashboard)/stock/page.tsx)) et Produits ([`src/app/(dashboard)/products/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/(dashboard)/products/page.tsx)) ne créent plus automatiquement 5 faux produits (« Bière Castel », « Burger Classic », « Whisky Jack Daniel's »...) à la première visite d'un établissement sans catalogue. Un établissement vide affiche désormais son véritable état vide (« Aucun produit trouvé ») au lieu de données factices qui donnaient l'impression d'un tableau statique. Les 3 catégories de base (Boissons, Nourriture, Services) et leurs modèles de caractéristiques restent créés automatiquement pour que le formulaire d'ajout de produit soit utilisable dès le premier usage. Le tableau de stock (`StockTable`), le calcul des KPIs (valeur totale, stock faible, rupture) et les boutons `+`/`−` d'ajustement (`StockAdjustmentModal` → `POST /inventory/movements/`) étaient déjà entièrement branchés à l'API — seule l'injection de données de démo faussait la perception d'une page statique.
- **Section Logs & Traçabilité Temps Réel (`/logs`) :** Ajout de la nouvelle section **Logs** dans la barre latérale ([`src/components/layout/Sidebar.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/components/layout/Sidebar.tsx)) sous *Abonnement*. Développement d'une interface complète et dynamique ([`src/app/(dashboard)/logs/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/(dashboard)/logs/page.tsx)) affichant les requêtes et actions en temps réel des utilisateurs via WebSockets, avec cartes statistiques (Total, 2xx, 4xx, 5xx), filtres interactifs (méthode, statut, recherche textuelle) et bouton d'**exportation Excel (.xlsx / .csv)**.

- **Gestion du Prix du Casier (`crate_price`) (Facultatif) :** Ajout du champ **Prix du casier (F) (Opt.)** dans les formulaires d'ajout et de modification de produit ([`src/app/(dashboard)/products/page.tsx`](file:///c:/mes%20projets/noxia%20folder/noxia-frontend/src/app/(dashboard)/products/page.tsx)) lorsque le mode de vente est défini sur *Par Casier*. Rechargement automatique de la valeur du casier à la modification et intégration synchrone dans les calculs de commandes.
- **Refonte Ergonomique & Visualisation des Statuts de Commande :** Refonte visuelle des badges de statut dans la table des commandes récentes (`/fournisseurs`) avec un design vibrant à haut contraste (Ambre pour *En attente*, Bleu Ciel pour *Expédiée*, Émeraude pour *Livrée* avec puces lumineuses). Positionnement fluide du menu déroulant (`z-50`).
- **Notification Toast & Spinner Dynamique sur Changement de Statut :** Intégration de `toast.promise` lors du changement de statut d'une commande fournisseur, affichant un spinner de chargement (*"Mise à jour du statut..."*) suivi d'un message explicite confirmant l'entrée automatique en stock lors de la livraison : *« ✅ Commande livrée ! Le stock du produit a été mis à jour automatiquement. »*.
