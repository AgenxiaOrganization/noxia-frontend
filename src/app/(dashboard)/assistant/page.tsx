...
import {
  ...
  Brain, Zap, TrendingUp, Package,
  Users, DollarSign, Lightbulb
} from 'lucide-react'
...
const suggestedQuestions = [
  ...
  'Qui est le meilleur serveur ce mois-ci ?',
  'Fais l\'inventaire des boissons alcoolisées.',
  'Génère le rapport de la semaine.',
  'Quelle est ma marge bénéficiaire ?',
  'Quelles caisses ont le plus vendu ?',
]
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs px-2 py-1 rounded-full flex items-center gap-1" style={{ background: 'rgba(34, 197, 94, 0.15)', color: '#22c55e' }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg) => (
            ...
            >
              <div className={`flex items-start gap-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                >
                  <p className="text-sm whitespace-pre-line" style={{ color: msg.sender === 'user' ? '#ffffff' : '#f1f5f9' }}>
                    {msg.message}
                  </p>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'rgba(51, 65, 85, 0.3)' }}>
                ...
                <span className="text-xs" style={{ color: '#94a3b8' }}>NOXIA réfléchit...</span>
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t" style={{ borderColor: '#334155' }}>
          <div className="flex gap-2">
            <textarea
              ...
              className="flex-1 rounded-lg px-4 py-2.5 text-white text-sm outline-none transition resize-none min-h-[44px] max-h-[120px]"
              style={{ 
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg text-white text-sm font-semibold transition disabled:opacity-50 flex items-center gap-2"
              style={{ 
                background: '#4f46e5',
          ...
          <p className="text-xs mt-2" style={{ color: '#64748b' }}>
            Appuyez sur <span className="font-mono text-white">Entrée</span> pour envoyer, <span className="font-mono text-white">Maj+Entrée</span> pour un saut de ligne
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-sm text-white">Questions suggérées</h3>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { icon: TrendingUp, label: 'CA & Ventes', color: '#22c55e' },
          { icon: Package, label: 'Stocks', color: '#3b82f6' },
          { icon: Users, label: 'Employés', color: '#8b5cf6' },
          { icon: DollarSign, label: 'Finances', color: '#f59e0b' },
          { icon: Zap, label: 'Alertes', color: '#ef4444' },
          { icon: Brain, label: 'Analyses', color: '#ec4899' },
        ].map((cap, i) => (
          <div 
            key={i}
          ...
          >
            ...
            <span className="text-xs text-center" style={{ color: '#94a3b8' }}>{cap.label}</span>
          </div>
        ))}
      </div>
    </div>
